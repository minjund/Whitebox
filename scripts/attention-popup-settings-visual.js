...
async function run() {
  ...
  const compactPopup = await win.webContents.executeJavaScript(`(() => {
    const card = document.querySelector('#attentionPopupSettingsCard').getBoundingClientRect();
    const copy = document.querySelector('.attention-popup-settings-copy').getBoundingClientRect();
    const toggle = document.querySelector('.attention-popup-switch').getBoundingClientRect();
    return { left: card.left, right: card.right, copyWidth: copy.width, toggleRight: toggle.right, viewport: innerWidth };
  })()`, true);
  assert.ok(compactPopup.left >= 0 && compactPopup.right <= compactPopup.viewport && compactPopup.toggleRight <= compactPopup.viewport);
  assert.ok(compactPopup.copyWidth >= 150, `Popup copy must remain readable: ${JSON.stringify(compactPopup)}`);
  ...
  await win.webContents.executeJavaScript("document.querySelector('#attentionPopupSettingsCard').scrollIntoView({ block: 'center' })", true);
