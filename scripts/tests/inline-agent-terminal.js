...
function createInlineHarness(root, options = {}) {
  ...
  const window = {
    ...
    WhiteboxRendererUtils: {
      ...
      canUseWritablePtySurface(candidate) {
        if (this.canForkCodexDesktopSession(candidate)) return true;
        if (candidate?.provider === 'codex' && candidate?.clientKind === 'codex-desktop') return false;
        return this.isWritableDirectSession(candidate)
          && candidate?.controlCapabilities?.pty === true
          && candidate?.presentation?.conversationSurface !== 'transcript';
      },
    },
...
function registerInlineAgentTerminalTests(context) {
  ...
  test('PTY 집중 모드는 담당 AI만 열고 기존 관제 선택을 바꾸지 않는다', async () => {
    ...
    const liveDesktopSession = {
      ...mainSession,
      id: 'codex:focus-live-desktop',
      externalId: 'focus-live-desktop',
    ...
    const readOnlySync = await liveDesktop.sync();
    ...
    const completedDesktopSession = {
      ...liveDesktopSession,
      id: 'codex:focus-completed-desktop',
      externalId: 'focus-completed-desktop',
      status: 'completed',
    };
    const completedDesktop = createInlineHarness(root, {
      session: completedDesktopSession,
      initialOpen: false,
      mountForAgent: async () => ({ ok: true, target }),
    });
    assert.equal(completedDesktop.enterFocus(completedDesktopSession.id), true,
      'canonical 완료 Codex Desktop 기록의 새 fork PTY 집중 모드를 차단했습니다.');
    assert.equal(completedDesktop.app.state.ptyFocusSessionId, completedDesktopSession.id);
  });
  ...
  test('긴 resume 중 사용자 조작은 완료 뒤 PTY focus를 취소한다', async () => {
    ...
    assert.equal(transition.resumeCalls.length, 1,
      'inline resume 진행 중 focus 전환이 같은 provider resume을 중복 시작했습니다.');
