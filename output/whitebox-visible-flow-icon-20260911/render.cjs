...
const expectedSourceHash = 'a338cb589935721dc00da71ea7d7457ab7fbb85cff90a62cc3ac6b2ebc68cd82';
const source = fs.readFileSync(path.join(root, 'upstream-brain-circuit.svg'));
  ...
  try {
    ...
    const exports = await win.webContents.executeJavaScript(`(async () => {
      ...
      for (const [index, color] of ['#202522', '#f6f4ee'].entries()) {
        ...
        c.fillText('AI의 작동 흐름을 들여다보다', cx, 325);
        c.globalAlpha = 1;
